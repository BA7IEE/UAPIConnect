...
function deriveRelayProfileFromFiles(profile: RelayProfile): RelayProfile {
  ...
  return {
    ...
    autoCompactLimit: hasPerModelWindows || modelMapHasOverrides(profile.modelAutoCompact)
...
function relayProfileHasPerModelWindows(profile: RelayProfile): boolean {
  return modelMapHasOverrides(profile.modelWindows) || (profile.modelList || "")
    .split(/[\r\n,]/)
    .some((model) => parseModelSuffix(model).window !== undefined);
}

function modelMapHasOverrides(value: string | undefined): boolean {
