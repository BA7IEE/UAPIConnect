...
#[tokio::test]
async fn launch_lifecycle_runs_enabled_maintenance_before_applying_fixed_provider_profile() {
    let temp = tempfile::tempdir().unwrap();
...
#[tokio::test]
async fn launch_lifecycle_applies_fixed_provider_profile_before_launching_codex() {
    let temp = tempfile::tempdir().unwrap();
    let app_dir = temp.path().join("Codex.app");
    std::fs::create_dir_all(&app_dir).unwrap();
    let status_store = StatusStore::new(temp.path().join("latest-status.json"));
    ...
    assert!(events.contains(&"apply-relay".to_string()));
    assert!(
        events.iter().position(|event| event == "apply-relay")
            < events.iter().position(|event| event == "launch:9229")
    );
...
#[tokio::test]
async fn launch_lifecycle_applies_fixed_provider_profile_with_duplicate_context_parent_tables() {
    let temp = tempfile::tempdir().unwrap();
    let app_dir = temp.path().join("Codex.app");
    std::fs::create_dir_all(&app_dir).unwrap();
    let status_store = StatusStore::new(temp.path().join("latest-status.json"));
