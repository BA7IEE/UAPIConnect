...
#[test]
fn per_model_percent_takes_priority_over_the_legacy_absolute_fallback() {
    let temp = tempfile::tempdir().unwrap();
    let profile = RelayProfile {
        id: "preview-compact".to_string(),
        model: "model-a".to_string(),
        model_list: "model-a\nmodel-b".to_string(),
        model_windows: r#"{"model-a":"1M"}"#.to_string(),
        model_auto_compact: r#"{"model-a":"85.123456%"}"#.to_string(),
        context_window: "512000".to_string(),
    ...
    let catalog: serde_json::Value = serde_json::from_slice(
        &std::fs::read(temp.path().join("model-catalogs/preview-compact.json")).unwrap(),
    )
    .unwrap();
    assert_eq!(catalog["models"][0]["context_window"], 1_000_000);
    assert_eq!(catalog["models"][0]["auto_compact_token_limit"], 851_235);
    assert_eq!(catalog["models"][1]["context_window"], 512_000);
    assert_eq!(catalog["models"][1]["auto_compact_token_limit"], 400_000);
}

#[test]
fn per_model_compact_without_windows_removes_only_its_managed_top_level_limit() {
    let temp = tempfile::tempdir().unwrap();
    let profile = RelayProfile {
        id: "preview-compact-only".to_string(),
        model: "model-a".to_string(),
        model_auto_compact: r#"{"model-a":"50%"}"#.to_string(),
        context_window: "512000".to_string(),
        auto_compact_limit: "400000".to_string(),
        relay_mode: RelayMode::PureApi,
        config_contents: "model = \"model-a\"\n".to_string(),
    ...
    assert!(!config.contains("model_auto_compact_token_limit"));
    let catalog: serde_json::Value = serde_json::from_slice(
        &std::fs::read(temp.path().join("model-catalogs/preview-compact-only.json")).unwrap(),
    )
    .unwrap();
...
#[test]
fn backfill_does_not_claim_handwritten_compact_override_for_per_model_percent() {
    let temp = tempfile::tempdir().unwrap();
    std::fs::write(
        temp.path().join("config.toml"),
    ...
    let mut profile = RelayProfile {
        model: "model-a".to_string(),
        model_auto_compact: r#"{"model-a":"80%"}"#.to_string(),
        ..RelayProfile::default()
    };
    codex_plus_core::relay_config::backfill_relay_profile_from_home(temp.path(), &mut profile)
        .unwrap();
    assert!(profile.auto_compact_limit.is_empty());
    assert!(
        profile
            .config_contents
            .contains("model_auto_compact_token_limit = 12345")
    );
}

#[test]
fn normalize_valid_toml_preserves_comments_and_multiline_table_like_text() {
    let source = "# user note\nmodel = \"model-a\"\n# codex-plus-external-responses-catalog-copy\nmodel_catalog_json = \"model-catalogs/user.json\"\nnote = '''\n[not_a_table]\nx = 1\nx = 2\n'''\n";
    assert_eq!(
        codex_plus_core::relay_config::normalize_config_text(source),
        source
    );
}
