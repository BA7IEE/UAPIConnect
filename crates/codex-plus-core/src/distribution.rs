...
mod tests {
    ...
    fn rust_constants_match_distribution_manifest() {
        let manifest: Value = serde_json::from_str(include_str!(
            "../../../distribution/uapi-connect.json"
        ))
        ...
        assert_eq!(manifest["managerDisplayName"].as_str(), Some(MANAGER_DISPLAY_NAME));
        ...
        assert_eq!(manifest["managerBundleId"].as_str(), Some(MANAGER_BUNDLE_ID));
        assert_eq!(manifest["urlScheme"].as_str(), Some(URL_SCHEME));
        assert_eq!(manifest["fixedProviderId"].as_str(), Some(FIXED_PROVIDER_ID));
        assert_eq!(manifest["fixedProviderName"].as_str(), Some(FIXED_PROVIDER_NAME));
        assert_eq!(manifest["fixedBaseUrl"].as_str(), Some(FIXED_BASE_URL));
        assert_eq!(manifest["helpUrl"].as_str(), Some(HELP_URL));
        assert_eq!(manifest["upstreamSourceUrl"].as_str(), Some(SOURCE_UPSTREAM_URL));
        assert_eq!(manifest["updateFeedUrl"].as_str(), Some(UPDATE_FEED_URL));
        assert_eq!(manifest["updateRepository"].as_str(), Some(UPDATE_REPOSITORY));
        assert_eq!(
            manifest["features"]["fixedProviderEdition"].as_bool(),
            Some(FIXED_PROVIDER_EDITION)
        );
        assert_eq!(manifest["features"]["adsEnabled"].as_bool(), Some(ADS_ENABLED));
        assert_eq!(
