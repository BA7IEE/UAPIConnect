...
fn normalize_duplicate_toml_text(contents: &str) -> String {
    // 合法 TOML 无需拆块。拆块会丢失根键的注释装饰，尤其是外部 catalog 副本的归属标记。
    // 也避免把多行字符串中的类似表头文本误判成真实表头。
    if let Ok(doc) = contents.parse::<DocumentMut>() {
        return normalize_optional_toml(doc);
    }
...
fn apply_model_catalog_to_config(
    ...
    // 先校验结构化配置，即使最终沿用外部 catalog，也不能静默忽略损坏的 JSON。
    ...
    let has_per_model_compact = entries
        .iter()
        .any(|entry| entry.auto_compact_percent.is_some());
    let has_per_model_overrides =
        has_metadata_overrides || has_per_model_window || has_per_model_compact;
    ...
    let needs_managed_catalog = has_per_model_overrides
    ...
    // 只接管自己的目录和已知 cc-switch 投影；用户外部目录与逐模型覆盖不能混用。
    ...
    if !needs_managed_catalog {
        // 清空逐模型覆盖后撤掉指针，但保留带归属标记的外部 Responses 兼容副本。
    ...
    let mut doc = parse_toml_document(&config_text)?;
    doc["model_catalog_json"] = toml_edit::value(catalog_relative);
    // profile 单值已实体化到 catalog 后，移除受管顶层 override；
    // raw TOML 中的手写值仍由用户掌握，不能因 profile 字段为空而误删。
    if has_per_model_window && !profile.context_window.trim().is_empty() {
        doc.as_table_mut().remove("model_context_window");
    }
    if (has_per_model_window || has_per_model_compact)
        && !profile.auto_compact_limit.trim().is_empty()
    {
...
fn validate_model_windows(model_windows: &HashMap<String, String>) -> anyhow::Result<()> {
    for (slug, value) in model_windows {
        let window = crate::model_suffix::parse_window_token(value)
            .ok_or_else(|| anyhow::anyhow!("model_windows 的模型 {slug} 窗口值无效：{value}"))?;
        if i64::try_from(window).is_err() {
            anyhow::bail!(
                "model_windows 的模型 {slug} 窗口数值过大：{value}，不能超过 {}",
                i64::MAX
            );
        }
    }
...
fn sync_context_limits_from_config(profile: &mut RelayProfile, config_text: &str) {
    ...
    if !model_map_has_overrides(&profile.model_auto_compact)
        && let Some(value) = root_positive_int_string(config_text, "model_auto_compact_token_limit")
    {
...
fn profile_has_per_model_windows(profile: &RelayProfile) -> bool {
    model_map_has_overrides(&profile.model_windows)
        || profile
            .model_list
            .split(['\r', '\n', ','])
...
fn model_map_has_overrides(serialized: &str) -> bool {
